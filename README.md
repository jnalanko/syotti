...
Suppose we have a set of reference sequences S_1, S_2, ... , S_n. The goal is to find a small set of bait strings B_1, ... , B_m, all of equal length L, such that the baits together cover every position of the reference sequences. Bait B_i is considered to cover position i of reference S_j if there is a length-L substring X of S_j spanning position i that is a d-Hamming neighbor of B_i
